...
shouldHaveSameStructureAs ::
  (HasServer api1 '[DefaultContext], HasServer api2 '[DefaultContext]) => Proxy api1 -> Proxy api2 -> Expectation
shouldHaveSameStructureAs p1 p2 =
  unless (sameStructure (makeTrivialRouter p1) (makeTrivialRouter p2)) $
    expectationFailure ("expected:\n" ++ unpack (layout p2) ++ "\nbut got:\n" ++ unpack (layout p1))

makeTrivialRouter :: (HasServer layout '[DefaultContext]) => Proxy layout -> Router ()
makeTrivialRouter p =
  route p defaultContext (emptyDelayed (FailFatal err501))
